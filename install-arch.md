...
##### Сохраняем Все настройки
 ```w``` - команда для сохранение настроек
...
### 3. Монтируем (Mount). Команда `mount` в Linux используется для подключения (монтирования) файловых систем или внешних устройств, таких как USB-накопители и диски, к дереву каталогов системы.
...
Базовая система:<br>
base, base-devel, linux, linux-headers: Минимальный набор для системы, инструменты разработки, ядро и заголовки для совместимости с драйверами.<br>
...
#### Генерируем locale-gen командой
```locale-gen```<br>
...
```sudo usermod -aG wheel,audio,video,optical,storage {username}```<br>
...
```sudo systemctl start dhcpcd```<br>
